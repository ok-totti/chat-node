...
socket.on('recieve', function(data) {
	...
	} else if (data.type === 'chat' && data.img != "") {
		var msg = data.value.replace(/[!@$%<>'"&|]/g, '');
		item.addClass('well well-lg').append($('<div>').text(msg)).append($('<img>').attr("src", '/files/'+data.img)).children('small').prepend(data.user + '：');
	} else if (data.type === 'chat' && data.img == ""){
		var msg = data.value.replace(/[!@$%<>'"&|]/g, '');
